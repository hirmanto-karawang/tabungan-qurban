...
const TENANT_SHEET_TEMPLATE = {
  ...
  // "kalkulator distribusi bagian sapi", lihat computeSurveyKalkulasi() di
  // app.html utk rumus estimasi tiap bagian. SENGAJA tidak ada kolom "wo" spt
  // RencanaDistribusi (daging) - budget-nya dibandingkan ke total akumulasi
  // (semua survey) langsung, bukan per-sapi. "jenis" isinya salah satu dari
  // 'tulang'/'jeroan' (satuan kg) atau 'kepala'/'kaki'/'buntut' (satuan pcs).
  RencanaDistribusiLain: ['id', 'jenis', 'alokasi', 'berat', 'qty', 'status', 'created_date'],
  // Work Order Sapi utk bagian NON-DAGING - realisasi alokasi PER SAPI dari
  // RencanaDistribusiLain (mis. sapi #3 dapat jatah 2 pcs Kepala ke alokasi
  // "Panti Asuhan"). Skema paralel ke DistribusiDaging tapi sheet terpisah
  // krn ada kolom "jenis" tambahan (dan supaya nama alokasi yang sama antara
  // Rencana Daging vs Rencana Bagian Lain tidak saling tabrakan waktu
  // dicocokkan). Dicocokkan ke RencanaDistribusiLain via (jenis + alokasi,
  // case-insensitive) sama seperti DistribusiDaging dicocokkan via alokasi
  // ke RencanaDistribusi - lihat loadWorkOrderList()/simpanWoQtyLain() di
  // app.html.
